    ...
    >
      ...
      >
        ...
        <el-breadcrumb-item
          ...
          :key="index"
          :to="'/index/gangweileixing?centerType=' + (centerType ? '1' : '0')"
        ...
        >
          ...
          >
            <el-button
              ...
              v-if="btnAuth('gangweileixing', '修改')"
              @click="editClick"
              >修改</el-button
            >
            <el-button
              ...
              v-if="btnAuth('gangweileixing', '删除')"
              @click="delClick"
              >删除</el-button
            >
            <el-button
              ...
              v-if="btnAuth('gangweileixing', '私聊') && detail.id != mid"
              @click="chatClick"
              >联系TA</el-button
            >
          </div>
...
export default {
  ...
  methods: {
    ...
    editClick() {
      this.$router.push(
        `/index/gangweileixingAdd?type=edit&&id=${this.detail.id}`,
      );
    },
    // 删除
    async delClick() {
      await this.$confirm("是否删除此岗位类型？")
        .then((_) => {
          this.$http
            .post("gangweileixing/delete", [this.detail.id])
            .then(async (res) => {
              if (res.data.code == 0) {
                this.$message({
                  type: "success",
