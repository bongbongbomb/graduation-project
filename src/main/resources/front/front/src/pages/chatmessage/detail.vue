    ...
    >
      ...
      >
        ...
        <el-breadcrumb-item
          ...
          :key="index"
          :to="'/index/chatmessage?centerType=' + (centerType ? '1' : '0')"
        ...
        >
          ...
          >
            ...
            <el-button
              ...
              v-if="btnAuth('chatmessage', '私聊') && detail.id != mid"
              @click="chatClick"
              >联系TA</el-button
            >
          </div>
...
export default {
  ...
  methods: {
    ...
    editClick() {
      this.$router.push(
        `/index/chatmessageAdd?type=edit&&id=${this.detail.id}`,
      );
    },
    // 删除
    async delClick() {
      await this.$confirm("是否删除此消息表？")
        .then((_) => {
          this.$http
            .post("chatmessage/delete", [this.detail.id])
            .then(async (res) => {
              if (res.data.code == 0) {
                this.$message({
                  type: "success",
