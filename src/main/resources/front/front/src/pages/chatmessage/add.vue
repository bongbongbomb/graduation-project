...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        ...
        content: [],
        format: [{ validator: this.$validate.isIntNumer, trigger: "blur" }],
        isread: [{ validator: this.$validate.isIntNumer, trigger: "blur" }],
      },
      centerType: false,
    };
  },
  ...
  methods: {
    ...
    info() {
      this.$http
        .get(`chatmessage/detail/${this.$route.query.id}`, {
          emulateJSON: true,
        })
        .then((res) => {
          if (res.data.code == 0) {
