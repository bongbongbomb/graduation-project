...
export default {
  ...
  methods: {
    ...
    info() {
      this.$http
        .get(`shixishenqing/detail/${this.$route.query.id}`, {
          emulateJSON: true,
        })
        .then((res) => {
          if (res.data.code == 0) {
    ...
    shenqingfujianUploadChange(fileUrls) {
      this.ruleForm.shenqingfujian = fileUrls.replace(
        new RegExp(this.$config.baseUrl, "g"),
        "",
      );
    },
