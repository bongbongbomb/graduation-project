    ...
    >
      ...
      >
        ...
        <el-breadcrumb-item
          ...
          :key="index"
          :to="'/index/shixishenqing?centerType=' + (centerType ? '1' : '0')"
    ...
    >
        ...
        >
          ...
          >
            ...
            <el-button
              ...
              @click="download(detail.shenqingfujian)"
              >点击下载</el-button
            >
          </div>
          <div
          ...
          >
            ...
            <el-button
              ...
              v-if="btnAuth('shixishenqing', '私聊') && detail.id != mid"
              @click="chatClick"
              >联系TA</el-button
            >
            <el-button
              ...
              v-if="btnAuth('shixishenqing', '录用通知')"
              @click="onAcross('luyongtongzhi', '', '', '', '')"
              type="warning"
              >录用通知</el-button
            >
          </div>
      ...
      >
        <el-tab-pane label="申请详情" name="first">
          <div v-html="detail.shenqingxiangqing"></div>
        </el-tab-pane>
      </el-tabs>
    </div>
    <div
...
export default {
  ...
  methods: {
    ...
    async onAcross(
      ...
      localStorage.setItem("crossTable", `shixishenqing`);
    ...
    backClick() {
      ...
      this.$router.push({ path: "/index/shixishenqing", query: params });
    },
    // 下载
    download(file) {
      if (!file) {
    ...
    editClick() {
      this.$router.push(
        `/index/shixishenqingAdd?type=edit&&id=${this.detail.id}`,
      );
    },
    // 删除
    async delClick() {
      await this.$confirm("是否删除此实习申请？")
        .then((_) => {
          this.$http
            .post("shixishenqing/delete", [this.detail.id])
            .then(async (res) => {
              if (res.data.code == 0) {
                this.$message({
                  type: "success",
