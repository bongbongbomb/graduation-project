    ...
    >
      ...
      >
        ...
        <el-breadcrumb-item
          ...
          :key="index"
          :to="'/index/friend?centerType=' + (centerType ? '1' : '0')"
        ...
        >
          ...
          >
            ...
            <el-button
              ...
              v-if="btnAuth('friend', '私聊') && detail.id != mid"
              @click="chatClick"
              >联系TA</el-button
            >
          </div>
  ...
  methods: {
    ...
    async delClick() {
      await this.$confirm("是否删除此好友表？")
        .then((_) => {
          this.$http
            .post("friend/delete", [this.detail.id])
            .then(async (res) => {
              if (res.data.code == 0) {
                this.$message({
                  type: "success",
