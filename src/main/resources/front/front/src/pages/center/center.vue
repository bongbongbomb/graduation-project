  ...
  >
    <div
      ...
      :style="{
        ...
        textAlign: 'center',
        background:
          'url(http://codegen.caihongy.cn/20240120/777684ebe3104d3cb040057bea6e001c.png) center center/100% 100% no-repeat',
        width: '100%',
        fontSize: '20px',
        lineHeight: '54px',
      }"
    ...
    >
      ...
      >
        ...
        ></span>
        <div :style="{ color: '#333', fontSize: '14px', display: 'none' }">
          学生账号
        </div>
        <div
          :style="{
      ...
      >
        ...
        ></span>
        <div :style="{ color: '#3554A4', fontSize: '15px', display: 'block' }">
          性别
        </div>
        <div
          :style="{ color: '#3554A4', textAlign: 'center', fontSize: '15px' }"
        >
          {{ sessionForm.xingbie }}
        </div>
      </div>
      ...
      >
        ...
        ></span>
        <div :style="{ color: '#3554A4', fontSize: '15px' }">手机号</div>
        <div
          :style="{ color: '#3554A4', textAlign: 'center', fontSize: '15px' }"
        >
          {{ sessionForm.shoujihao }}
        </div>
      </div>
    </div>
    ...
    >
      <el-tab-pane label="个人中心">
        ...
        >
          ...
          >
            <el-input
              v-model="sessionForm.xueshengzhanghao"
              placeholder="学生账号"
              readonly
            ></el-input>
          </el-form-item>
          ...
          >
            <el-input
              v-model="sessionForm.xueshengxingming"
              placeholder="学生姓名"
            ></el-input>
          </el-form-item>
          <el-form-item
      ...
      <el-tab-pane label="聊天记录" style="width: 100%" v-if="changeHasChat()">
        <div
          class="z-box"
          :style="{ width: '70%', padding: '20px', margin: '0 auto' }"
        >
          <div
      ...
      <div class="chat-content" id="chat-content">
        <div v-bind:key="item.id" v-for="item in chatList">
          ...
          <div v-else class="left-content">
            <img
              :src="
                nowfpic ? baseUrl + nowfpic : require('@/assets/avator.png')
              "
              alt=""
              style="
                width: 30px;
...
export default {
  ...
  data() {
    return {
      ...
      chatTimer: null,
      hasChatList: ["xuesheng", "qiye", "shixigangwei"],
    };
  },
  created() {
    let menus = menu.list();
    ...
    if ("xuesheng" == this.userTableName && this.rules["xueshengzhanghao"]) {
      this.rules["xueshengzhanghao"].push({
        required: true,
        message: "请输入学生账号",
        trigger: "blur",
      });
    } else if (
      "xuesheng" == this.userTableName &&
      !this.rules["xueshengzhanghao"]
    ) {
      this.$set(this.rules, "xueshengzhanghao", [
        { required: true, message: "请输入学生账号", trigger: "blur" },
      ]);
    }
    if ("xuesheng" == this.userTableName && this.rules["mima"]) {
      this.rules["mima"].push({
        required: true,
        message: "请输入密码",
        trigger: "blur",
      });
    ...
    if ("xuesheng" == this.userTableName && this.rules["xueshengxingming"]) {
      this.rules["xueshengxingming"].push({
        required: true,
        message: "请输入学生姓名",
        trigger: "blur",
      });
      ...
      this.$set(this.rules, "xueshengxingming", [
        { required: true, message: "请输入学生姓名", trigger: "blur" },
      ]);
    }
    if ("xuesheng" == this.userTableName) {
      this.$set(this.rules, "shoujihao", [
        {
          required: false,
