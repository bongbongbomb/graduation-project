    ...
    >
      ...
      >
        ...
        <el-breadcrumb-item
          ...
          :key="index"
          :to="'/index/luyongtongzhi?centerType=' + (centerType ? '1' : '0')"
        ...
        >
          ...
          >
            ...
            <el-button
              ...
              v-if="btnAuth('luyongtongzhi', '私聊') && detail.id != mid"
              @click="chatClick"
              >联系TA</el-button
            >
            <el-button
              ...
              v-if="btnAuth('luyongtongzhi', '实习情况')"
              @click="onAcross('shixiqingkuang', '', '', '', '')"
              type="warning"
              >实习情况</el-button
            >
          </div>
...
export default {
  ...
  methods: {
    ...
    async onAcross(
      ...
      localStorage.setItem("crossTable", `luyongtongzhi`);
    ...
    backClick() {
      ...
      this.$router.push({ path: "/index/luyongtongzhi", query: params });
    },
    // 下载
    download(file) {
      if (!file) {
    ...
    editClick() {
      this.$router.push(
        `/index/luyongtongzhiAdd?type=edit&&id=${this.detail.id}`,
      );
    },
    // 删除
    async delClick() {
      await this.$confirm("是否删除此录用通知？")
        .then((_) => {
          this.$http
            .post("luyongtongzhi/delete", [this.detail.id])
            .then(async (res) => {
              if (res.data.code == 0) {
                this.$message({
                  type: "success",
