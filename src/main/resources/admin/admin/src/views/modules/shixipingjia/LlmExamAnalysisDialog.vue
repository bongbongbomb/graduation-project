...
<script>
	export default {
		...
		computed: {
			isInternship() {
				return String(this.mode || '').toLowerCase() === 'internship'
			},
			dialogTitle() {
			...
			scoreCardTitle() {
				return this.isInternship ? '实习评分占比' : '得分占比'
			},
			pieCardTitle() {
				if (this.isInternship) {
			...
			radarCardTitle() {
				return this.isInternship ? '能力维度雷达' : '题型能力雷达'
			},
			barsCardTitle() {
				return this.isInternship ? '维度得分与达成率' : '题型得分与正确率'
			},
			topCardTitle() {
				return this.isInternship ? '待改进 TOP5' : '失分 TOP5'
			},
			topValueLabel() {
				return this.isInternship ? '严重度' : '失分'
			},
			topEmptyText() {
				return this.isInternship ? '暂无可统计的待改进项' : '暂无可统计的客观题失分'
			},
			textCardTitle() {
				return this.isInternship ? '大模型实习分析' : '大模型文字分析'
			},
			innerVisible: {
				get() {
					return this.visible
		...
		methods: {
			...
			fetchLlmAnalysis(force) {
				if (!this.isInternship && (!this.paper || !this.paper.id)) {
					return
				}
				if (this.llmLoading) {
					return
				...
				const base = this.isInternship ? this.buildInternshipBaseData() : this.buildLlmBaseData()
				this.llmData = base
				this.llmAnalysisHtml = ''
				const prompt = this.isInternship ? this.buildInternshipPrompt(base) : this.buildLlmPrompt(base)
			...
			exportReport() {
				...
				const title = (this.isInternship ? (base.title || `实习报告分析-${base.xueshengxingming || ''}`) : (base.papername || '大模型分析')) || '报告'
