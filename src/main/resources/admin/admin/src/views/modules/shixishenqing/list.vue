<template>
	<div class="main-content" :style='{"padding":"20px 30px","background":"#f9fbfd","flexDirection":"column","display":"flex"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","alignItems":"center","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('shixishenqing','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia10" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","fontWeight":"600","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('shixishenqing','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"0"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="shenqingfujian" label="申请附件">
						<template slot-scope="scope">
							<el-button v-if="scope.row.shenqingfujian" type="text" size="small" @click="download(scope.row.shenqingfujian)">下载</el-button>
                            <span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('shixishenqing','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"15px","color":"#fff","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('shixishenqing','录用通知')" @click="luyongtongzhiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"15px","color":"#212529","height":"40px"}'></span>
								录用通知
							</el-button>
							<el-button class="edit" v-if=" isAuth('shixishenqing','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('shixishenqing','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<luyongtongzhi-cross-add-or-update v-if="luyongtongzhiCrossAddOrUpdateFlag" :parent="this" ref="luyongtongzhiCrossaddOrUpdate"></luyongtongzhi-cross-add-or-update>
...
import luyongtongzhiCrossAddOrUpdate from "../luyongtongzhi/add-or-update";
	export default {
		...
		methods: {
			...
			luyongtongzhiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
		...
		shixishenqingstatusChange(e,row){
			if(row.status==0){
