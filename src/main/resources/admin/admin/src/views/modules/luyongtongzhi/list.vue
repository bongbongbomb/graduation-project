<template>
	<div class="main-content" :style='{"padding":"20px 30px","background":"#f9fbfd","flexDirection":"column","display":"flex"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","alignItems":"center","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('luyongtongzhi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia10" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","fontWeight":"600","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('luyongtongzhi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('luyongtongzhi','录用结果统计')" type="success" @click="chartDialog1()">
			...
			<div :style='{"width":"100%","padding":"0"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('luyongtongzhi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"15px","color":"#fff","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('luyongtongzhi','实习情况')" @click="shixiqingkuangCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"15px","color":"#212529","height":"40px"}'></span>
								实习情况
							</el-button>
							<el-button class="edit" v-if=" isAuth('luyongtongzhi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('luyongtongzhi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<shixiqingkuang-cross-add-or-update v-if="shixiqingkuangCrossAddOrUpdateFlag" :parent="this" ref="shixiqingkuangCrossaddOrUpdate"></shixiqingkuang-cross-add-or-update>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="luyongzhuangtaiChart1" style="width:100%;height:600px;"></div>
...
import shixiqingkuangCrossAddOrUpdate from "../shixiqingkuang/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				shixiqingkuangCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			shixiqingkuangCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			shixiqingkuangCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.shixiqingkuangCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','luyongtongzhi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.shixiqingkuangCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.shixiqingkuangCrossaddOrUpdate.init(row.id,type);
				});
			},
		...
		chartDialog1() {
			...
			this.$nextTick(()=>{
				var luyongzhuangtaiChart1 = echarts.init(document.getElementById("luyongzhuangtaiChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						luyongzhuangtaiChart1.setOption(option);
						  //根据窗口的大小变动图表
						window.onresize = function() {
							luyongzhuangtaiChart1.resize();
						};
					}
				});
			})
		...
		getDataList() {
			...
			if(this.searchForm.luyongzhuangtai!='' && this.searchForm.luyongzhuangtai!=undefined){
				params['luyongzhuangtai'] = this.searchForm.luyongzhuangtai
			}
		...
		luyongtongzhistatusChange(e,row){
			if(row.status==0){
				row.passwordwrongnum = 0
			}
			this.$http({
				url:'luyongtongzhi/update',
				method:'post',
				data:row
			}).then(res=>{
				if(row.status==1){
