	...
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-if="type!='info'&& !ro.shixibaogao" label="实习报告" prop="shixibaogao">
					<file-upload
						tip="点击上传实习报告"
						action="file/upload"
						:limit="1"
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.shixibaogao" label="实习报告" prop="shixibaogao">
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 15px","margin":"0 20px 0 0","color":"rgba(255, 255, 255, 1)","outline":"none","borderRadius":"4px","background":"#6aab9c","width":"auto","lineHeight":"40px","fontSize":"16px","fontWeight":"400","height":"40px"}' type="text" size="small" @click="download($base.url+ruleForm.shixibaogao)">下载</el-button>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="!ruleForm.shixibaogao" label="实习报告" prop="shixibaogao">
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 15px","margin":"0 20px 0 0","color":"rgba(255, 255, 255, 1)","outline":"none","borderRadius":"4px","background":"#6aab9c","width":"auto","lineHeight":"40px","fontSize":"16px","fontWeight":"400","height":"40px"}' type="text" size="small">无</el-button>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="实习评价" prop="shixineirong">
					<editor 
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.shixineirong" label="实习评价" prop="shixineirong">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.shixineirong"></span>
                </el-form-item>
		...
		info(id) {
			this.$http({
				...
				if (data && data.code === 0) {
					...
					this.ruleForm.shixineirong = this.ruleForm.shixineirong.replace(reg,'../../../springboot8d6m43dj/upload');
				} else {
					this.$message.error(data.msg);
				}
			});
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `shixiqingkuang/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shixiqingkuangCrossAddOrUpdateFlag = false;
											this.parent.search();
