<template>
	<div class="main-content" :style='{"padding":"20px 30px","background":"#f9fbfd","flexDirection":"column","display":"flex"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","alignItems":"center","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('shixipingjia','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia10" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","fontWeight":"600","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('shixipingjia','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('shixipingjia','学生实习评分统计')" type="success" @click="chartDialog1()">
			...
			<div :style='{"width":"100%","padding":"0"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('shixipingjia','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"15px","color":"#fff","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="view" v-if=" isAuth('shixipingjia','查看')" type="success" @click="openInternshipReport(scope.row)">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"15px","color":"rgb(106, 171, 156)","height":"40px"}'></span>
								生成报告
							</el-button>
							<el-button class="edit" v-if=" isAuth('shixipingjia','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('shixipingjia','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="shixipingfenChart1" style="width:100%;height:600px;"></div>
...
import LlmExamAnalysisDialog from "./LlmExamAnalysisDialog.vue";
	export default {
		data() {
			return {
				searchForm: {
		...
		methods: {
			...
			openInternshipReport(row) {
				...
				this.llmInternship = {
					...
					shixitupian: row.shixitupian,
					shixipingfen: row.shixipingfen,
					pingjiashijian: row.pingjiashijian,
					qiyezhanghao: row.qiyezhanghao,
					fuzeren: row.fuzeren,
				}
				this.$http({
					url: `shixipingjia/info/${row.id}`,
					method: "get",
				}).then(({ data }) => {
					if (data && data.code === 0) {
						this.llmInternship = data.data
					} else {
						this.$message.error(data.msg || '获取实习评价详情失败')
					}
				}).catch(() => {
					this.$message.error('获取实习评价详情失败')
				})
			},
			contentStyleChange() {
		...
		chartDialog1() {
			...
			this.$nextTick(()=>{
				var shixipingfenChart1 = echarts.init(document.getElementById("shixipingfenChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						shixipingfenChart1.setOption(option);
						  //根据窗口的大小变动图表
						window.onresize = function() {
							shixipingfenChart1.resize();
						};
					}
				});
			})
		...
		shixipingjiastatusChange(e,row){
			if(row.status==0){
				row.passwordwrongnum = 0
			}
			this.$http({
				url:'shixipingjia/update',
				method:'post',
				data:row
			}).then(res=>{
				if(row.status==1){
