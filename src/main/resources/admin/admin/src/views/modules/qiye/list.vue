<template>
	<div class="main-content" :style='{"padding":"20px 30px","background":"#f9fbfd","flexDirection":"column","display":"flex"}'>
			...
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","alignItems":"center","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"margin":"0 10px 0 0","display":"flex"}' >
					<div :style='{"margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"16px","fontWeight":"500","height":"40px"}' class="item-label">企业账号</label>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="420" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="view" v-if=" isAuth('qiye','查看')" type="success" @click="openCompanyReputation(scope.row)">
		...
		<llm-exam-analysis-dialog
			mode="company"
			:visible.sync="companyReputationVisible"
			:company="companyReputationCompany"
			:reviews="companyReputationReviews"
		></llm-exam-analysis-dialog>
	...
	export default {
		data() {
			return {
				...
				companyReputationVisible: false,
				companyReputationCompany: {},
				companyReputationReviews: [],
		...
		methods: {
			...
			openCompanyReputation(row) {
				this.companyReputationCompany = row || {}
				this.companyReputationReviews = []
				this.companyReputationVisible = true
				if (this.companyReputationLoading) return
				this.companyReputationLoading = true
				const qiyezhanghao = row && row.qiyezhanghao ? String(row.qiyezhanghao) : ''
				const qiyemingcheng = row && row.qiyemingcheng ? String(row.qiyemingcheng) : ''
				const params = {
					page: 1,
					limit: 200,
					sort: 'dengjishijian',
				...
				this.$http({
					...
					this.companyReputationReviews = list
				}).catch(() => {
					this.companyReputationReviews = []
				}).finally(() => {
