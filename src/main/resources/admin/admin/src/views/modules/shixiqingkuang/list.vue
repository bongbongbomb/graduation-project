<template>
	<div class="main-content" :style='{"padding":"20px 30px","background":"#f9fbfd","flexDirection":"column","display":"flex"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","alignItems":"center","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"margin":"0 10px 0 0","display":"flex"}' >
					<div :style='{"margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"16px","fontWeight":"500","height":"40px"}' class="item-label">学生姓名</label>
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('shixiqingkuang','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia10" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","fontWeight":"600","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('shixiqingkuang','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"0"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="shixitupian" width="200" label="实习图片">
						<template slot-scope="scope">
							<div v-if="scope.row.shixitupian">
								<img v-if="scope.row.shixitupian.substring(0,4)=='http'" :src="scope.row.shixitupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.shixitupian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.shixitupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.shixitupian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="shixibaogao" label="实习报告">
						<template slot-scope="scope">
							<el-button v-if="scope.row.shixibaogao" type="text" size="small" @click="download(scope.row.shixibaogao)">下载</el-button>
                            <span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('shixiqingkuang','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"15px","color":"#fff","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('shixiqingkuang','实习报告')" @click="shixipingjiaCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"15px","color":"#212529","height":"40px"}'></span>
								实习报告
							</el-button>
							<el-button class="edit" v-if=" isAuth('shixiqingkuang','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('shixiqingkuang','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<shixipingjia-cross-add-or-update v-if="shixipingjiaCrossAddOrUpdateFlag" :parent="this" ref="shixipingjiaCrossaddOrUpdate"></shixipingjia-cross-add-or-update>
...
import shixipingjiaCrossAddOrUpdate from "../shixipingjia/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				shixipingjiaCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			shixipingjiaCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.shixipingjiaCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','shixiqingkuang');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.shixipingjiaCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.shixipingjiaCrossaddOrUpdate.init(row.id,type);
				});
			},
		...
		shixiqingkuangstatusChange(e,row){
			if(row.status==0){
				row.passwordwrongnum = 0
			}
			this.$http({
				url:'shixiqingkuang/update',
				method:'post',
				data:row
			}).then(res=>{
				if(row.status==1){
